...
class MS_TCN2_Modified(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, attention=False,
                 lstm=False, lstm_att=False, ridge_reg=False, num_layers_lstm_att=1, upsample=True, aggregate=False):
        ...
        self.Rs = nn.ModuleList(
            [copy.deepcopy(
                Refinement(num_layers=num_layers_R, num_f_maps=num_f_maps, num_classes=num_classes, dim=num_classes,
                           attention=attention, lstm=lstm, lstm_att=lstm_att, num_layers_lstm_att=num_layers_lstm_att,
                           aggregate=aggregate))
                for s in
                range(num_R)])
...
class Refinement(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes, attention=False, lstm_att=False, lstm=False,
                 upsample=False, downsample_factor=2, num_layers_lstm_att=1, aggregate=False):
        ...
        self.lstm = lstm
        self.aggregate = aggregate
        self.num_layers_lstm_att = num_layers_lstm_att
    ...
    def forward(self, x, current_layer, total_layers):
        ...
        if current_layer >= total_layers - self.num_layers_lstm_att:  # Amount of layers to perform the new architecture
            if self.att:
                ...
                if self.lstm_att:
                    out_att = torch.cat((out, out_att), dim=1)
                    out_att = out_att.permute(0, 2, 1)
                    out_att, _ = self.lstm_layer(out_att)  # apply LSTM layer

                    out_att = self.conv_out(out_att.permute(0, 2, 1))  # (batch_size, num_classes, seq_length)
                    if self.aggregate:
                        out = self.conv_out(out)
                        out_att = torch.cat((out, out_att), dim=0)
                    return out_att
                else:
...
class Trainer:
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, split,
                 model_type='mstcn2',
                 transformer_params={},
                 concat_kinematic_data=False,
                 kinematic_features_path="/datashare/APAS/kinematics_npy",
                 num_layers_lstm_att=1,
                 weighted=False, class_weights=None,
                 lstm=False, lstm_att=False, ridge_reg=False, attention=False, aggregate=False):
        """
        ...
        ridge_reg: add Ridge regularization term
        ...
        if model_type == 'mstcn2':
            if lstm_att:
                self.model = MS_TCN2_Modified(num_layers_PG=num_layers_PG, num_layers_R=num_layers_R, num_R=num_R,
                                              ...
                                              attention=attention, lstm_att=lstm_att, lstm=lstm, ridge_reg=ridge_reg,
    ...
    def clear_ml_reporter(self, clogger, epoch, epoch_loss_train, batch_gen_train, epoch_loss_val, batch_gen_val,
        ...
        clogger.report_scalar("Validation Accuracies - " + self.fold,
                              iteration=epoch + 1,
                              value=(float(correct_val) / total_val))
        clogger.report_scalar("Validation Edit Score - " + self.fold,
                              iteration=epoch + 1,
                              value=np.mean(edit_score_val))
        clogger.report_scalar("Train Edit Score - " + self.fold,
                              iteration=epoch + 1,
    ...
    def train(self, save_dir, batch_gen_train, batch_gen_val, num_epochs, batch_size, learning_rate, device, clogger,
              lambda_l2, accumulation_steps=5):
        self.model.train()
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            while batch_gen_train.has_next():
                ...
                if (batch_idx + 1) % accumulation_steps == 0:
                    ...
                    batch_loss /= accumulation_steps  # We accumulated loss over couple of steps
                    if self.ridge_reg:
                        l2_regularization_loss = self.l2_regularization_loss(lambda_l2)
                        batch_loss += l2_regularization_loss
                    batch_loss.backward()
                    optimizer.step()
                    batch_loss = 0
                ####################################################################################################
